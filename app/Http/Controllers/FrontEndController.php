...
class FrontEndController extends Controller
{
    ...
    private function customDomainMode(){
        ...
        if (!$theConfig) {
            $theConfig=Config::where('value', $domain)->first();
            // dd($theConfig);
        }
    ...
    public function whatsappMode()
    {
        if (config('settings.disable_landing')) {
            ...
            $featured_vendors=Restorant::where('active',1)->where('is_featured',0)->get()->shuffle();
            $restorant = Restorant::first()->only(['subdomain']);
             //dd($restorant);
            $demoLink="#";
...
$a = $featured_vendors;
            ...
            foreach ($a as $b) {
                ...
                $tz[$k]= $b->getConfig('time_zone',config('app.timezone'));
                $now = new \DateTime('now',new \DateTimeZone($tz[$k]));
                ...
                $formatter->setTimeZone($tz[$k]);
                $bh[$k] = $b->getBusinessHours();
                $st[$k] = $bh[$k]->isClosed()?$formatter->format($bh[$k]->nextOpen($now)):null;
                $ct[$k] = $bh[$k]->isOpen()?$formatter->format($bh[$k]->nextClose($now)):null;
                $k++;
            }
    ...
    public function restorant($alias)
    {
        ...
        if ($restorant && $restorant->active == 1) {
           ...
           /*$theme_number = $restorant->getConfig('template_theme','');
           
           switch($theme_number) {
                case 0: 
                   $viewFile = 'restorants.show1';
                   break;
                case 1:
                    $viewFile = 'restorants.grocery';
                    break;
                case 2:
                    $viewFile = 'restorants.show3';
                case 3:
                    $viewFile = 'restorants.show2';
                    break;
                default: 
                    $viewFile = 'restorants.show1';
        ...
        if ($doWeHaveMinimumPrice) {
            $min_price = $restorant->getConfig('min_delivery_price');
        } else {
            $min_price = null;
        }
        ...
        if (!empty($min_price)) {
           ...
           $viewData=[
                ...
                'doWeHaveMinimumPrice' => $doWeHaveMinimumPrice,
                'minPrice' => $min_price,
                'restorant' => $restorant,
