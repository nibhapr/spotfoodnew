...
@section('content')
<div id="minPrice" hidden>{{ $minPrice }}</div>
...
<h5 class="modal-title" id="add-address">{{ __('your cart') }}</h5>
</div>
...
<p class="price-1">@{{ item.attributes.friendly_price }}</p>														
        ...
        <div id="totalPrices" v-cloak>
          ...
          <p v-if="totalPrice" class="mb-1 text-capitalize">{{ __('Subtotal') }}<span class="float-right text-dark">@{{ totalPriceFormat }}</span></p>
	   ...
	   @if(config('app.isft')||config('settings.is_whatsapp_ordering_mode'))
		  <p v-if="totalPrice&&delivery" class="mb-1 text-capitalize">{{ __('delivery fee') }}  <span class="float-right text-dark">@{{deliveryPriceFormated }}</span></p>
            <hr />
			...
			<h6 v-if="totalPrice" class="font-weight-bold mb-0 text-uppercase">{{ __('total') }}<span
